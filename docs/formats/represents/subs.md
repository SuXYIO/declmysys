...
All commands will be parsed by subs before running.
...
`homedir` is probably `/home/username` if your username is `username`. (Unless you set a homedir manually, the `os` package'll probably correctly get the path anyway)
...
- `disable_homedir_subs`: whether to disable the special homedir substitution feature (won't disable `{HOME}`, don't worry), default `false`
- `rules`: set of subs rules for any string parsed, default empty
